...
class EdgeServer:
    ...
    def aggregate(self):
      total_samples = 0
      weighted_params = None  # Ensure initialization
      ...
      if total_samples == 0 or weighted_params is None:
          raise ValueError("Aggregation failed: No valid parameters received!")
...
def client_affordable_workload():
    ...
    return np.random.normal(mu_k, sigma_k)  # Sample workload
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        # ✅ Get k1 dynamically from config
        num_epochs = config.get("k1", 1)
        print(f"Client {self.cid}: Training for {num_epochs} epochs (Affordable Workload: {self.affordable_workload:.2f})")

        for epoch in range(num_epochs):  # Use k1 for local updates
...
########################################
def compute_training_rounds(client: FlowerClient, base_k1=60):
    """
    ...
    """
    return max(5, int(base_k1 * (client.affordable_workload / 60)))  # Scale k1 based on workload
...
def HierFL(args, trainloaders, valloaders, testloader):
    ...
    metrics = {'rounds': [], 'accuracy': [], 'train_time': []}

    # ✅ Ensure `fit_config()` is defined **before** strategy
    def fit_config(server_round: int):
        return {"server_round": server_round, "k1": args['k1']}  # Pass default k1

    def client_fn(cid: str):
        device = edge_devices[int(cid)]
        client = device.get_client()
        return client  # ✅ Flower expects only `client`, not extra parameters
    ...
    strategy = fl.server.strategy.FedAvg(
        ...
        on_fit_config_fn=fit_config  # ✅ Ensures k1 is dynamically set
    )
